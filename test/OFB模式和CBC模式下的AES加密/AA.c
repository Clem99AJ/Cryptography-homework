...
void AES(int ch) {
    ...
    int** c = NULL;
    int* c1 = (int*)malloc(sizeof(int) * MAX);
    unsigned char* c2 = (unsigned char*)malloc(sizeof(char) * MAX);
    int** c3 = NULL;
    unsigned char* m = (unsigned char*)malloc(sizeof(char) * MAX);
    int* m1 = (int*)malloc(sizeof(int) * MAX);
    unsigned char* key = (unsigned char*)malloc(sizeof(char) * KEYMAX);
    ...
    char *IV = (char*)malloc(sizeof(char) * KEYMAX);
    int* IV_1 = (int*)malloc(sizeof(int) * KEYMAX);
    RE_0:
    printf("---------------------\n");
    printf("|  ==AES加密算法==  |\n");
    printf("|   1、Zero填充     |\n");
...
RE_2:
    ...
    {
        case 1:
            c = GetC(m, key2, IV_1,ch);
            printf("AES加密的结果是：\n");
            DisPlayKEY_Nx16(c, strlen(m) / KEYMAX);
            break;
        case 3:
            //初始化需要输出用的数组
            c3 = (int**)malloc(sizeof(int*) * strlen(m) / KEYMAX);
            for (int i = 0; i < strlen(m) / KEYMAX; i++) {
                c3[i] = (int*)malloc(sizeof(int) * KEYMAX);
                memset(c3[i], 0, sizeof(int) * KEYMAX);
            }

            for (int i = 0; i < strlen(m) / KEYMAX;i++) {
                if (i == 0) {
                    c = GetC(IV, key2, IV_1, ch);
                }
                else {
                    c = GetC(c2, key2, IV_1, ch);
                }
                
                //IV加密后异或明文
                for (int j = 0; j < KEYMAX;j++) {
                    c1[j] = c[0][j] ^ m1[i * 16 + j];
                    c3[i][j] = c[0][j] ^ m1[i * 16 + j];
                }
...
unsigned char* Zero(unsigned char* m) {
    int len = strlen(m);
    int q = len / KEYMAX;
    int r = len % KEYMAX;
...
unsigned char* X923(unsigned char* m) {
    int len = strlen(m);
    int q = len / KEYMAX;
    int r = len % KEYMAX;
    for (int i = 0; i < KEYMAX - r; i++) {
...
unsigned char* PKCS7(unsigned char* m) {
    int len = strlen(m);
    int q = len / KEYMAX;
    int r = len % KEYMAX;
    for (int i = 0; i < KEYMAX - r; i++) {
...
unsigned char* ISO10126(unsigned char* m) {
    int len = strlen(m);
    int q = len / KEYMAX;
    int r = len % KEYMAX;
    srand(time(NULL));
...
unsigned char* GetMatrixKey(unsigned char* key) {
    unsigned char* key1 = (unsigned char*)malloc(sizeof(char) * KEYMAX);
    memset(key1,'0', sizeof(char) * KEYMAX);
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
...
int** GetSonKey(unsigned char* key) {
    int** key1 = (int**)malloc(sizeof(int*) * 4);
    for (int i = 0; i < 4; i++) {
        key1[i] = (int*)malloc(sizeof(int) * 44);
        memset(key1[i], 0, sizeof(int) * 44);
    ...
    switch (ch)
    {
        case 1:
            ...
            for (int i = 0; i < q; i++) {
                printf("第%d组加密：\n", i + 1);
                if (i == 0) {
                    c[i] = GetC1(m1[i], key, IV,ch);
                }
                else
                    c[i] = GetC1(m1[i], key, c[i - 1],ch);
            }
            break;
        case 3:
            //OFB分组加密(即AES默认加密)
...
int* GetC1(int* m, int** key,int *XX,int ch) {
...
void DisPlayMatrix(unsigned char* XX) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            printf("%c ", *((XX + i * 4) + j));
        }
